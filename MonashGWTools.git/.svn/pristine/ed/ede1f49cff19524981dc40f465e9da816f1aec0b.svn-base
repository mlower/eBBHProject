...
def freq_damping_obs(MM, jj, ell=2, mm=2, nn=0):
   ...
   '''
   MM = tools.m_sol_to_geo(MM)
   omega,tau = freq_damping(MM, jj, ell, mm, nn)
   f = omega / (2*np.pi)
   f =  tools.freq_geo_to_Hz(f)
   tau = tools.time_geo_to_s(tau)
   return f,tau
...
def freq_damping(MM, jj, ell=2, mm=2, nn=0):
    ...
    ## load data from Berti's file -- must be in the same directory
    #    data_file = './fitcoeffsWEB.dat'
    #    data_file = '/home/ethrane/repositories/MonashGWTools/fitcoeffsWEB.dat'
    data_file = os.path.join(os.path.dirname(__file__), 'fitcoeffsWEB.dat')
...
def amplitude(m1, m2, ell=2, mm=2):
    ...
    see equations (5) -- (8) from Gossan et al. (2012) (note there is a difference between the arXiv paper and the real paper)
    m1, m2 are component masses of the progenitor
    '''

    from tools import m12_to_symratio
    ...
    if mm == 1:
        ...
        print 'something'
        #warnings.warn("aaaaahhhhh: you've selected an ell or mm that is not appropriate for the amplitude calculation")
...
def final_mass_spin(m1, m2):
    ...
    m1 = float(m1)  # convert to floats just in case entered as integers
    m2 = float(m2)
